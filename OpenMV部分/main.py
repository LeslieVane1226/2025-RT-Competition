...
while(True):
    ...
    filename = f"image_{photo_counter:03d}.jpg"
    if value==1:
        img.save(filename)
        led.on()
        time.sleep_ms(50)
    ...
    for i, detection_list in enumerate(net.predict([img], callback=fomo_post_process)):
        ...
        if current_time -last_print_time >= print_interval:
            ...
            for x, y, w, h, score in detection_list:
                ...
                img.draw_circle((center_x, center_y, 15), color=(255,0,0),line_width=5)
            last_print_time=current_time
    output_pin.value(1 if has_detection else 0)
